create procedure pa_autor_sumaypromedio2
  @promedio decimal(6,2) output
  as 
   select distinct PrecioCordobas=case
 ...
 execute pa_autor_sumaypromedio2  @p output
...
,T1.WhsCode,
 ...
 when t0.SlpCode = 1 then 'Cristhian Peña'
 when t0.SlpCode = 2 then 'Eliseo García'
 when t0.SlpCode = 3 then 'Freddy Gutiérrez'
  when t0.SlpCode = 5 then 'Misael Rivera'
 when t0.SlpCode = 6 then 'Ronald Nicaragua'
 when t0.SlpCode = 8 then 'Martín Paz'
 when t0.SlpCode = 10 then 'Bühler'
